...
def Ridge_Corr(num_feats: int,
    ...
    X_train = X_train[:, np.argpartition(np.nan_to_num(correlation_coefficients), -num_feats)[-num_feats:]]
    ...
    X_test = X_test[:, np.argpartition(np.nan_to_num(correlation_coefficients), -num_feats)[-num_feats:]]
...
if __name__ == '__main__':
    ...
    for repeat in range(100):
        ...
        ks = [1, 2, 3, 4, 5, 10, 15, 20, 30, 40, 50, 60, 70, 80, 90, 100, 150, 200, num_rois]
        ...
        for trait in traits:
            ...
            for j, region in enumerate(ks):
                ...
                y_hats_test = []
                for k in range(5):
                    mae_train, r_train, mae_test, r_test, y_train_hat, y_train, y_test_hat, y_test = Ridge_Corr(region, test_data[k], train_data[k], correlation_coefficients_folds[k], normalization=norm)
